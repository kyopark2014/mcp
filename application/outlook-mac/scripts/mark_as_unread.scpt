            ...
            if messageIDInput contains "," then
                ...
                repeat with currentID in messageIDs
                    ...
                    on error errMsg
                        ...
                        log "Error marking message " & currentID & " as unread: " & errMsg
                    end try
                end repeat
                
                return "Successfully marked " & markedCount & " of " & (count of messageIDs) & " emails as unread"
            else
                -- Single message ID
                set theMessage to message id messageIDInput
                set is read of theMessage to false
                return "Email marked as unread successfully"
            end if
        on error errMsg
            return "Error marking email as unread: " & errMsg
        end try
    end tell
end run
