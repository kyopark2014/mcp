...
> **CRITICAL: Skill Directory Rule**
> - Skill development folders MUST be created under `skills/` (i.e. `skills/<skill-name>/`).
> - NEVER create skill folders under `artifacts/`. The `artifacts/` directory is only for generated output files (PDFs, images, etc.) and packaged `.skill` files—not for skill source directories.
> - When using `write_file` to create skill files, always use paths like `skills/<skill-name>/SKILL.md`, `skills/<skill-name>/scripts/example.py`, etc.
...
The `--path skills` argument ensures the skill is created at `skills/<skill-name>/`, which is the standard skill installation directory. Never use `--path artifacts` or any other path.
...
Save the packaged `.skill` file to the `artifacts/` directory:

```bash
scripts/package_skill.py skills/<skill-name> artifacts
```
